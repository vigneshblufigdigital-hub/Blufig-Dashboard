...
export function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Look up custom password: check Firestore document first, fallback to local storage, fallback to 'password'
      const savedCustomPassword = matchedUser.password || localStorage.getItem('blufig_custom_password_' + matchedUser.email.toLowerCase()) || 'password';

      // Verify custom password
      const isValidPassword = (password && password === savedCustomPassword) ||
        (password && !matchedUser.password) ||
        (!password && savedCustomPassword === 'password');

      if (isValidPassword) {
        // If they selected client tab but matched an agency user, switch mode automatically
        ...
        // Sync password back to Firestore if it was missing
        if (!matchedUser.password) {
          const effectivePassword = password || savedCustomPassword;
          const updatedUser = { ...matchedUser, password: effectivePassword };
          await saveDocToFirestore('users', updatedUser);
        }
