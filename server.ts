...
`;
  ...
  apiRouter.post("/send-email", async (req, res) => {
    try {
      ...
      let portalUrl = req.body.appUrl || process.env.APP_URL;
      if (!portalUrl) {
        const reqOrigin = req.get('origin') || req.get('referer');
        if (reqOrigin) {
          try {
            const parsedUrl = new URL(reqOrigin);
            portalUrl = parsedUrl.origin + parsedUrl.pathname.replace(/\/api.*$/, '').replace(/\/+$/, '');
          } catch {
            portalUrl = reqOrigin.split('?')[0].replace(/\/api.*$/, '').replace(/\/+$/, '');
          }
        }
      }
      if (!portalUrl) {
        const host = req.get('x-forwarded-host') || req.get('host') || "blufigdigital.co";
        if (host.includes("194.238.17.57") || host.includes("blufigdigital.co")) {
          portalUrl = "https://blufigdigital.co/BluOps";
        } else {
    ...
    <tr>
      ...
      if (type === "assignment") {
        ...
        const bodyContent = `
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #27272a;">You have been assigned to a new task on the <strong>BluFig Operations Portal</strong> by <strong>${creator?.name || "a team member"}</strong>:</p>
          ...
          <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 18px; margin: 20px 0;">
            ...
            <table role="presentation" style="width: 100%; border-collapse: collapse; font-size: 13px;">
              ...
              <tr>
                ...
                <td style="padding: 6px 0; color: #0f172a; font-weight: bold;">${creator?.name || "Team Member"}${creator?.email ? ` (${creator.email})` : ""}</td>
              </tr>
              <tr>
