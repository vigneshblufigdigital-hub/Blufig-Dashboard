...
import { Project, Task, UserProfile, UserRole, ADMIN_ROLES, ClientInvoice, isSuperAdmin, hasPermission } from '@/src/types';
...
export function ProjectBoard({ 
  ...
  const canCreateProject = isSuperAdmin(currentUser) || hasPermission(currentUser, 'canCreateProject') || (currentUser && ADMIN_ROLES.includes(currentUser.role));
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <CardHeader className="pb-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <CardTitle className="text-xl font-bold tracking-tight mt-2 flex items-center justify-between group/title">
                    <div className="flex items-center gap-2">
                      ...
                      {project.websiteUrl && (
                        <a 
                          ...
                          className="inline-flex items-center text-zinc-400 hover:text-brand-secondary transition-colors"
                    ...
                    {onEditProject && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 ml-2 text-zinc-400 hover:text-brand-secondary hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-md shrink-0 opacity-80 group-hover/title:opacity-100 transition-opacity"
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger
                        className={cn(
                          ...
                          "h-8 w-8 text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 rounded-lg animate-none cursor-pointer flex items-center justify-center"
                        )}
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
                      ...
                      <DropdownMenuContent 
                        ...
                        onMouseDown={(e) => e.stopPropagation()}
                      >
                        <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-400 px-3 py-1.5">Project Actions</div>
                        <DropdownMenuSeparator />
                        ...
                        {isAdminUser && onUpdateProjectAM && project.accountManagerId !== currentUser?.id && (
                        ...
                        {isAdminUser && onUpdateProjectStatus && (
                          <>
                            <DropdownMenuSeparator />
                            <div className="text-[9px] uppercase font-bold tracking-widest text-zinc-400 px-3 py-1.5">Set Status</div>
      ...
      {canCreateProject && (
        <button 
          ...
          className="h-[300px] border-2 border-dashed border-zinc-200 rounded-2xl flex flex-col items-center justify-center text-zinc-400 hover:bg-zinc-100 hover:border-zinc-300 transition-all space-y-4 w-full"
        >
