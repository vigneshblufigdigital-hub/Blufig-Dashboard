...
export function UserProfileView({ usersList, onUpdateUsers, onOpenRoleSwitcher, targetUserId }: UserProfileViewProps) {
  ...
  // Automatically select searched user in Teammates Directory if targetUserId is provided
  useEffect(() => {
    if (targetUserId) {
      const foundUser = usersList.find(u => u.id === targetUserId || u.email === targetUserId);
      if (foundUser) {
        setSelectedFolder('tree');
        setSelectedTreeMember(foundUser);
        setSearchTermTree('');
        setTimeout(() => {
          const cardEl = document.getElementById(`teammate-card-${foundUser.id}`);
          if (cardEl) {
  ...
  // Auto-fill custom avatar if it's an image URL or base64 data string
  React.useEffect(() => {
    if (user.avatarUrl) {
      setProfileAvatar(user.avatarUrl);
      if (user.avatarUrl.startsWith('http') || user.avatarUrl.startsWith('/') || user.avatarUrl.startsWith('data:')) {
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="w-16 h-16 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 flex items-center justify-center overflow-hidden shrink-0">
                        {(() => {
                          const activeAvatar = customAvatarUrl || profileAvatar || user.avatarUrl;
                          const isImg = activeAvatar && (activeAvatar.startsWith('http') || activeAvatar.startsWith('/') || activeAvatar.startsWith('data:'));
                          if (isImg) {
                            return (
                              <img 
                                src={activeAvatar} 
                          ...
                          return <span className="text-3xl select-none">{activeAvatar || user.name.charAt(0)}</span>;
                        })()}
                      </div>
                        ...
                        <label className="flex flex-col items-center justify-center h-16 px-4 py-2 bg-white dark:bg-zinc-950 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-xl cursor-pointer hover:border-brand-secondary hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-all">
                          ...
                          <input 
                            ...
                            onChange={(e) => {
                              ...
                              if (file) {
                                compressImageToDataUrl(file, 300, 0.82).then(async (resultStr) => {
                                  ...
                                  try {
                                    ...
                                    console.error("Failed to persist avatar to Firestore:", err);
                                  }

                                  const updatedList = usersList.map(u => u.id === user.id ? updatedUser : u);
                                  onUpdateUsers(updatedList);

                                  toast.success("Profile picture uploaded & saved to database successfully!");
                                }).catch((err) => {
                                  console.error(err);
