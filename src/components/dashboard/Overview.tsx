...
import { ADMIN_ROLES, UserRole, isSuperAdmin, hasPermission } from '../../types';
...
export function Overview({ 
  ...
  onNavigateToTasks?: (filters?: { dateFilter?: string; statusFilter?: string; priorityFilter?: string; projectId?: string }) => void,
  ...
  const tasksDueTodayCount = visibleTasks.filter(t => t.dueDate === todayStr && !['Done', 'Approved', 'Cancelled', 'done', 'approved', 'cancelled'].includes(t.status)).length;
  ...
  startOfWeekObj.setDate(todayObj.getDate() - currentDayIdx); // Sunday
  startOfWeekObj.setHours(0, 0, 0, 0);

  const endOfWeekObj = new Date(startOfWeekObj);
  endOfWeekObj.setDate(startOfWeekObj.getDate() + 6); // Saturday
  endOfWeekObj.setHours(23, 59, 59, 999);

  const tasksDueThisWeekCount = visibleTasks.filter(t => {
    const isCompleted = ['Done', 'Approved', 'Cancelled', 'done', 'approved', 'cancelled'].includes(t.status);
    if (isCompleted) return false;
    if (!t.dueDate) return false;
    const taskDate = new Date(t.dueDate + 'T00:00:00');
    if (isNaN(taskDate.getTime())) return false;
    return taskDate >= startOfWeekObj && taskDate <= endOfWeekObj;
  }).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
        <StatCard 
          ...
          tooltip="Click to view all active projects"
        />
        <StatCard 
          title="Tasks Due Today" 
          value={tasksDueTodayCount.toString()} 
          change={`${overdueTasksCount} Overdue`} 
          changeType={overdueTasksCount > 0 ? "warning" : "neutral"}
          icon={Clock} 
          color="orange" 
          onClick={() => {
            if (onNavigateToTasks) onNavigateToTasks({ dateFilter: 'today' });
          }}
          onBadgeClick={(e) => {
            e.stopPropagation();
            if (onNavigateToTasks) onNavigateToTasks({ dateFilter: 'overdue' });
          }}
          tooltip="Click to view tasks due today (or click badge for overdue)"
        />
        <StatCard 
          title="Due This Week" 
          value={tasksDueThisWeekCount.toString()} 
          change="This Week" 
          icon={Calendar} 
          color="purple" 
          onClick={() => {
            if (onNavigateToTasks) onNavigateToTasks({ dateFilter: 'this-week' });
          }}
          tooltip="Click to view tasks due this week"
        />
        <StatCard 
          title="Completed (7d)" 
          ...
          color="emerald" 
          onClick={() => {
            if (onNavigateToTasks) onNavigateToTasks({ statusFilter: 'Done' });
          }}
        ...
        <StatCard 
          ...
          tooltip="Click to view delivery health details"
        />
      </div>
              ...
              {myActiveTasks.slice(0, 6).map(task => {
                ...
                return (
                  <div 
                    ...
                    onClick={() => {
                      if (onClickTask) {
                        onClickTask(task.id, task.projectId);
                      } else if (onClickProject && task.projectId) {
                        onClickProject(task.projectId);
                      }
                    }}
                    className="p-4 rounded-xl border border-zinc-200/50 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 flex flex-col justify-between space-y-4 hover:border-orange-500/80 dark:hover:border-orange-500/80 hover:shadow-md transition-all cursor-pointer group relative"
                  >
                    <div className="space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <span 
                          onClick={(e) => {
                            if (task.projectId && onClickProject) {
                              e.stopPropagation();
                              onClickProject(task.projectId);
                            }
                          }}
                          className="text-[9px] font-bold text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 uppercase tracking-wider truncate max-w-[140px] flex items-center gap-1 cursor-pointer transition-colors"
                          title="Filter by Project"
                        >
                          <Folder className="w-3 h-3 text-zinc-400 group-hover:text-orange-500 transition-colors" />
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <h4 className="text-xs font-bold text-zinc-950 dark:text-zinc-50 leading-snug group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors flex-1">
                          {task.name}
                        </h4>
                        <ArrowRight className="w-3.5 h-3.5 text-zinc-400 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all shrink-0 mt-0.5" />
                      </div>
        ...
        <Card className="lg:col-span-2 border-zinc-200/50 dark:border-zinc-800 bg-white dark:bg-zinc-900/50">
          <CardHeader className="pb-3 border-b border-zinc-100 dark:border-zinc-800/50 flex flex-row items-center justify-between space-y-0">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              {onAddProjectClick && (isSuperAdmin(user) || hasPermission(user, 'canCreateProject') || (user && ADMIN_ROLES.includes(user.role))) && (
                <Button
                  size="sm"
                  onClick={onAddProjectClick}
                  className="h-8 bg-brand-secondary hover:bg-brand-secondary/90 text-white font-bold text-xs px-3 rounded-lg flex items-center gap-1 shrink-0 cursor-pointer shadow-sm transition-all"
                >
...
function StatCard({ 
  ...
  value: string;
  change: string;
  changeType?: "warning" | "success" | "neutral";
  icon: any;
  color: string;
  onClick?: () => void;
  ...
  return (
    <Card 
      ...
      title={tooltip}
      className={cn(
        "border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 transition-all duration-200 group relative select-none",
        onClick && "cursor-pointer hover:border-orange-500/70 dark:hover:border-orange-500/70 hover:shadow-md hover:-translate-y-0.5"
      )}
    >
      <CardContent className="pt-5 pb-5 px-4">
        <div className="flex items-center justify-between">
          <div className={cn("p-2 rounded-xl transition-transform group-hover:scale-105", colorMap[color] || colorMap.blue)}>
          ...
          <div className="flex items-center gap-1.5">
            <Badge 
              ...
              className={cn(
                ...
                onBadgeClick && "cursor-pointer hover:bg-orange-100 hover:text-orange-900 hover:border-orange-300 dark:hover:bg-orange-900/60 dark:hover:text-orange-200",
                changeType === 'warning' || change.includes('Overdue') ? "text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/30" :
                change.startsWith('+') || change === 'Excellent' || change === 'Live Sync' ? "text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900/60 bg-emerald-50 dark:bg-emerald-950/30" :
                "text-zinc-500 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900"
              )}
            >
              {change}
            </Badge>
        ...
        <div className="mt-3.5">
          <h3 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-zinc-100">{value}</h3>
          <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-extrabold uppercase tracking-wider mt-1">{title}</p>
        </div>
      </CardContent>
    </Card>
  );
