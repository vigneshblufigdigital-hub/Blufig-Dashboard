...
export function ClientPortal({ 
  ...
  React.useEffect(() => {
    if (highlightedTaskId) {
      setActiveTab('tasks');
      setTimeout(() => {
        const rowEl = document.getElementById(`client-task-row-${highlightedTaskId}`);
        if (rowEl) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <Card className="border-zinc-200/60 shadow-sm rounded-2xl overflow-hidden">
                ...
                <CardContent className="p-6 space-y-6">
                  {clientProjects.map((project) => {
                    ...
                    return (
                      <div key={project.id} className="p-5 rounded-2xl border border-zinc-100 bg-white/50 space-y-4">
                        ...
                        {am && (
                          <div className="flex items-center justify-between pt-3 bg-zinc-50 rounded-xl p-3 border border-zinc-100">
                            ...
                            <Button 
                              ...
                              className="h-8 text-xs font-bold font-mono text-zinc-600 hover:text-zinc-900"
                            >
                            ...
                            <TableRow key={task.id} className="hover:bg-zinc-50/50">
                              ...
                              <TableCell className="py-3.5">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-xs font-medium text-zinc-700">{expert?.name || 'Assigned Agent'}</span>
                                </div>
                              </TableCell>
                              <TableCell className="py-3.5">
                                <Badge className={cn(
                                  ...
                                  task.status === TaskStatus.REVIEW ? "bg-purple-500 text-white" : "bg-zinc-100 text-zinc-500"
                                )}>
                                  {task.status}
                                </Badge>
                              </TableCell>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <Card className="border border-zinc-200/60 dark:border-zinc-800/80 shadow-lg rounded-2xl overflow-hidden bg-white/85 dark:bg-zinc-900/85 backdrop-blur-md">
                <CardHeader className="bg-zinc-50/60 dark:bg-zinc-950/40 pb-4 border-b border-zinc-100 dark:border-zinc-800">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <CardDescription className="text-[10px] mt-0.5 font-semibold text-zinc-500 dark:text-zinc-400">Need assistance? Your dedicated BluFig support squad is standing by.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="flex flex-col gap-2 pt-1 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest leading-none">
                    <div className="flex items-center justify-between">
                      <span>Primary Email Desk:</span>
                      <span className="font-mono text-zinc-600 dark:text-zinc-300 select-all font-extrabold text-[11px] lowercase">connect@blufig.digital</span>
                    </div>
                    <div className="flex items-center justify-between">
              ...
              <Card className="border-zinc-200/60 shadow-sm rounded-2xl overflow-hidden">
                ...
                <CardContent className="p-4 space-y-3">
                  {clientInvoices.slice(0, 2).map((inv) => (
                    <div key={inv.id} className="p-3 rounded-xl border border-zinc-100 flex items-center justify-between gap-3 bg-white">
                      <div>
                  ...
                  <TableBody>
                    {filteredClientTasks.map(task => {
                      ...
                      return (
                        <TableRow 
                          key={task.id} 
                          id={`client-task-row-${task.id}`}
                          className={cn(
                            "border-zinc-50 group hover:bg-zinc-50/50 transition-colors",
                            task.id === highlightedTaskId && "ring-2 ring-orange-500 bg-orange-500/5 dark:bg-orange-950/20 transition-all scale-[1.01]"
                          )}
                        >
                          <TableCell className="pl-8 py-4">
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] font-medium text-zinc-400 uppercase mt-0.5">{task.type} • Priority: {task.priority}</span>
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="flex items-center space-x-2">
                              <Avatar className="w-6 h-6">
                                <AvatarImage src={expert?.avatarUrl} />
                                <AvatarFallback className="text-[9px] bg-zinc-100 flex items-center justify-center font-bold">
                                  {expert?.name ? expert.name.split(' ').map(n=>n[0]).join('') : '?'}
                                </AvatarFallback>
                              </Avatar>
                          ...
                          <TableCell className="text-xs font-mono font-semibold text-zinc-500 pr-8">
                            {new Date(task.dueDate).toLocaleDateString()}
                          </TableCell>
                        </TableRow>
                      );
